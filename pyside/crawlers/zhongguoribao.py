...
class ChineseDayNews:
    ...
    def get_news_list(self, code=None):
        ...
        for element in div_elements:
            ...
            x = {
                ...
                "cover_url": cover_url,
                "date_str": datetime.strptime(date_str, "%Y-%m-%d %H:%M").strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }
            result.append(x)
    ...
    def get_news_info(self, news, category=None):
        ...
        content_html = etree.HTML(content)
        content_div_elements = content_html.xpath('//div[@id="Content"]')
        if not content_div_elements:
            # 尝试其他可能的容器
            content_div_elements = content_html.xpath(
                '//div[@class="article"]'
            ) or content_html.xpath('//div[@class="content"]')
        if not content_div_elements:
            return None
        content_div = content_div_elements[0]
        ...
        if len(article_info) < 50:
            return None
        try:
            date_str = datetime.strptime(news["date_str"], "%Y-%m-%d %H:%M").strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        except:
