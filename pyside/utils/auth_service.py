...
class AuthService:
    ...
    @staticmethod
    def check_login_status(state: str) -> str | None:
        """检查登录状态，返回token或None"""
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
            "Accept": "application/json",
    ...
    @staticmethod
    def login_with_password(open_id: str, password: str) -> str | None:
        """使用 open_id + password 登录，返回 token"""
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
            "Accept": "application/json",
        ...
        try:
            response = requests.post(
                f"{AuthService.BASE_URL}/user/login/",
                json={"open_id": open_id, "password": password},
                headers=headers,
                timeout=10,
                verify=False,
            )
