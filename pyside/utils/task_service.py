...
from crawlers.spider_all import (
    get_lsit,
    get_lsit_info,
    is_less_than_2_minutes,
    is_less_than_user_minutes,
)
...
class TaskService:
            ...
            for info in HotSpotService.PLATFORM_CONFIG:
                ...
                for c in info["children"]:
                    if c["classify"] in targets:
                        code = c["code"]
                        dict_ = {"platform": platform, "code": code, "account": account}
                        task.append(dict_)
    ...
    def start_production_task(self, log, item):
        ...
        try:
            ...
            if is_publish:
                ...
                is_create, article = self.produce_content(
    ...
    def produce_content(self, topic, selected_model, api_key, prompt, _id, is_not_full):
        ...
        try:
            ...
            # 尝试生成文章
            article, usetokens, enable = article_create(
                topic, selected_model, api_key, prompt, is_not_full
            )
    ...
    def publish_content(self, article, cookie, img_list, platform, _id):
        ...
        if result["status"]:
            # 跟新文章状态
            partial_update_news(
                _id, {"status": 2, "published_at": now.strftime("%Y-%m-%d %H:%M:%S")}
            )
        else:
                ...
                try:
                    ...
                    for t in spider_task:
                        ...
                        try:
                            ...
                            time_l = [
                                h["published_at"] for h in history if h["published_at"] is not None
                            ]
                            max_time = max(time_l)
                            if is_less_than_user_minutes(max_time):
                                log.append_log("当前账号发布间隔时间不足xx，暂时跳过")
                                continue
                            else:
                                log.append_log("当前账号发布间隔已满足√√，开始监控")
                        except:
                            pass
