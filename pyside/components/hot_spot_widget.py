...
from api.api_all import create_news, delete_news, get_news_list
...
class HotSpotWidget(QWidget):
    ...
    def create_article_card(self, article):
        ...
        config_btn.clicked.connect(
            lambda checked, a=article: self.cancel_account_config(a)
            if a.get("configured_account")
        ...
        try:
            ...
            try:
                ...
                if "已存在" in str(e):
                    ...
                    popup = MessagePopup(
                        "该文章已被其他人配置", parent=self, message_type="warning"
                    )
                    popup.show()
                    ...
                    article["configured_other"] = {"id": 0}
                    popup = MessagePopup(
                        "已达到账号配置上限，请调整", parent=self, message_type="warning"
                    )
                    popup.show()
                else:
                    account["configured_other"] = {"id": 0}
